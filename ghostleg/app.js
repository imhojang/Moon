...
function readOnlyInput() {
    for (
        let i = 0;
        i < document.getElementsByClassName("inputBox").length;
        i++
    ) {
        document.getElementsByClassName("inputBox")[i].readOnly = true;
        if (i < numPlayers) {
            document.getElementsByClassName("player")[i].style.cursor =
                "pointer";
            document.getElementsByClassName("target")[i].style.cursor =
                "default";
        }
    }
}
...
function showInputs() {
    ...
    for (let i = 0; i < numPlayers; i++) {
        ...
        inputTargetNameArr[i].style.top = `${$canvasTop +
            $canvas.height -
            37}px`;
...
function drawGhostLegs(lineId) {
    ...
    if ($canvas.getContext) {
        ...
        while (currentY !== player[i].yCoord) {
            ...
            if (lineIndex > 0) {
                if (
                    randomCrossPointArrays[lineIndex - 1].includes(
                        player[i].yCoord
                    )
                ) {
